...
export function AccelerationZChart({ rideData }: AccelChartProps) {
  if (!rideData) {
    return (
      <Card className="h-80 flex items-center justify-center bg-muted/20">
        <div className="text-center text-muted-foreground">
          <Activity className="w-12 h-12 mx-auto mb-2 opacity-50" />
          <p>Select a ride to view Acc Z</p>
        </div>
      </Card>
    );
  }
    ...
    <Card className="h-80 p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold">Acceleration Z</h3>
        </div>
      </div>
      <div className="h-60">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <Tooltip 
              ...
              formatter={(value)=> [`${(value as number).toFixed?.(2) ?? value} m/s²`, 'Acc Z']}
            />
            <ReferenceLine y={0} stroke="hsl(var(--muted-foreground))" />
            <Line type="monotone" dataKey="acc" stroke="hsl(var(--secondary-foreground))" strokeWidth={2}
              dot={(props:any)=>props?.payload?.error ? (<circle cx={props.cx} cy={props.cy} r={3} fill="hsl(var(--secondary))" stroke="white" strokeWidth={1} />): null} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </Card>
